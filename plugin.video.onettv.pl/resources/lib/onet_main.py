...
class Main:
	def __init__( self ):
		...
		# Facet
		#
		listitem = xbmcgui.ListItem( "Facet", iconImage = "DefaultFolder.png" )
		xbmcplugin.addDirectoryItem( handle = int(sys.argv[ 1 ]), url = '%s?facet&po_co=%s' % ( sys.argv[ 0 ], "Facet" ), listitem=listitem, isFolder=True)

		#
		# Wnetrza
		#
		listitem = xbmcgui.ListItem( "Wnętrza", iconImage = "DefaultFolder.png" )
		xbmcplugin.addDirectoryItem( handle = int(sys.argv[ 1 ]), url = '%s?wnet&po_co=%s' % ( sys.argv[ 0 ], "Wnętrza" ), listitem=listitem, isFolder=True)

		#
		# Zdrowie
		#
		listitem = xbmcgui.ListItem( "Zdrowie", iconImage = "DefaultFolder.png" )
		xbmcplugin.addDirectoryItem( handle = int(sys.argv[ 1 ]), url = '%s?zdro&po_co=%s' % ( sys.argv[ 0 ], "Zdrowie" ), listitem=listitem, isFolder=True)

		#
		# Nowe Technologie
		#
		listitem = xbmcgui.ListItem( "Nowe Technologie", iconImage = "DefaultFolder.png" )
		xbmcplugin.addDirectoryItem( handle = int(sys.argv[ 1 ]), url = '%s?nowtech&po_co=%s' % ( sys.argv[ 0 ], "Nowe Technologie" ), listitem=listitem, isFolder=True)
